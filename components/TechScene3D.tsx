...
function OctahedronWithTrail({
  ...
  useFrame((state) => {
    if (meshRef.current) {
      meshRef.current.position.x =
        position[0] + Math.sin(state.clock.elapsedTime * 0.5) * 0.8;
      meshRef.current.position.y =
        position[1] + Math.cos(state.clock.elapsedTime * 0.4) * 0.5;
  ...
  return (
    <Trail width={1.5} length={5} color="#1e3a5f" attenuation={(t) => t * t}>
      <mesh ref={meshRef} position={position}>
  ...
  useFrame((state) => {
    if (groupRef.current) {
      groupRef.current.rotation.z =
        Math.sin(state.clock.elapsedTime * 0.1) * 0.1 + tilt;
...
const particleData = (() => {
  ...
  for (let i = 0; i < PARTICLE_COUNT; i++) {
    ...
    const r = 10 + seededRandom(seed1) * 10;
    const theta = 2 * Math.PI * seededRandom(seed2);
    ...
    sizes[i] = seededRandom(seed4) * 2;

    // Orange and Blue colors, but brighter
    if (seededRandom(seed5) > 0.5) {
      // Bright Orange
      colors[i3] = 1.0;
      colors[i3 + 1] = 0.6;
      colors[i3 + 2] = 0.2;
...
function InteractiveCamera() {
  ...
  useFrame((state) => {
    // Access camera from state parameter instead of useThree hook
    const cam = state.camera;
    cam.rotation.x += (targetRotation.current.x - cam.rotation.x) * 0.02;
    cam.rotation.y += (targetRotation.current.y - cam.rotation.y) * 0.02;
  });
