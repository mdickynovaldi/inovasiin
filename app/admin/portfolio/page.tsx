...
const categories = ['All', 'Virtual Reality', 'Augmented Reality', 'Web Development', '3D Modeling', 'Motion Graphics']
...
export default function PortfolioListPage() {
  ...
  async function deletePortfolio(id: string) {
    ...
    try {
      ...
      await supabase.from('portfolio_technologies').delete().eq('portfolio_id', id)
      await supabase.from('portfolio_testimonials').delete().eq('portfolio_id', id)
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-[#1e3a5f]/60">Kelola semua project portfolio Anda</p>
        </div>
        <Link href="/admin/portfolio/new">
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-gradient-to-r from-[#f97316] to-[#ea580c] text-white font-medium shadow-lg shadow-orange-500/20"
          >
      ...
      <div className="flex items-center gap-6 text-sm">
        <span className="text-[#1e3a5f]/60">
          Total: <span className="font-semibold text-[#1e3a5f]">{portfolios.length}</span>
        </span>
        <span className="text-[#1e3a5f]/60">
          Featured: <span className="font-semibold text-[#f97316]">{portfolios.filter(p => p.is_featured).length}</span>
        </span>
        <span className="text-[#1e3a5f]/60">
          Showing: <span className="font-semibold text-[#1e3a5f]">{filteredPortfolios.length}</span>
        </span>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredPortfolios.map((portfolio, index) => (
            ...
            >
              {/* Thumbnail */}
              <div className="relative h-48 bg-gradient-to-br from-[#1e3a5f] to-[#0f2847]">
              ...
              {/* Content */}
              <div className="p-5">
                <p className="text-sm text-[#f97316] font-medium mb-1">{portfolio.industry}</p>
                <h3 className="font-bold text-[#1e3a5f] text-lg mb-2 line-clamp-1">{portfolio.title}</h3>
                <p className="text-sm text-[#1e3a5f]/60 line-clamp-2 mb-4">{portfolio.subtitle}</p>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <div className="flex items-center gap-2">
                    ...
                    >
                      {portfolio.is_featured ? <Star className="w-4 h-4" /> : <StarOff className="w-4 h-4" />}
                    </button>
                  </div>
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-100">
          ...
          <h3 className="text-xl font-semibold text-[#1e3a5f] mb-2">
            {searchQuery || selectedCategory !== 'All' ? 'Tidak Ditemukan' : 'Belum Ada Portfolio'}
          </h3>
          ...
          {!searchQuery && selectedCategory === 'All' && (
            <Link href="/admin/portfolio/new">
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                className="inline-flex items-center gap-2 px-6 py-3 rounded-xl bg-gradient-to-r from-[#f97316] to-[#ea580c] text-white font-medium shadow-lg shadow-orange-500/20"
              >
                <Plus className="w-5 h-5" />
                Tambah Portfolio
              </motion.button>
          ...
          >
            ...
            >
              <div className="text-center">
                ...
                <h3 className="text-xl font-bold text-[#1e3a5f] mb-2">Hapus Portfolio?</h3>
                <p className="text-[#1e3a5f]/60 mb-6">
                  Tindakan ini tidak dapat dibatalkan. Portfolio dan semua data terkait akan dihapus permanen.
                </p>
